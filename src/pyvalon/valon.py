...
class Valon(object):
    ...
    def __init__(self, dev, baud):
        self.ser = serial.Serial(
            dev, baud, timeout=0.5, rtscts=False, xonxoff=False
        )
...
class V500X(object):
    ...
    def CheckReadBack(self, b, length, c):
        """
        Description:
            Check the read back data - 1: the data length; 2 - checksum.
        Inputs:
            b (bytearray): the read back data
            length (int): the expected data length
            c (byte): the expected checksum value
        """
        if len(b) != length or len(c) != 1:
            print(
                "Read Back data incorrect: The length should be %d, while it's %d"
                % (length, len(b))
            )
            return False
    ...
    def GetReference(self):
        ...
        b = self._read(4)
        c = self._read(1)
        if not self.CheckReadBack(b, 4, c):
            return
    ...
    def GetOptions(self, synth):
        ...
        b = self._read(24)
        c = self._read(1)
        if not self.CheckReadBack(b, 24, c):
            return
    ...
    def GetLabel(self, synth):
        ...
        b = self._read(16)
        c = self._read(1)
        if not self.CheckReadBack(b, 16, c):
            return
    ...
    def GetFreq(self, synth, verbose=False):
        ...
        try:
            freq = (
                (regs["ncount"] + float(regs["frac"]) / regs["mod"])
                * EPDF
                / regs["dbf"]
            )
    ...
    def GetPhaseLock(self, synth, verbose=False):
        ...
        b = self._read(1)
        c = self._read(1)
        if not self.CheckReadBack(b, 1, c):
            return
        if synth == "A":
            mask = 0x20
        elif synth == "B":
