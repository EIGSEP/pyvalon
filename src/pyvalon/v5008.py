...
def GetSynth(args):
    if args.synth is None:
        print(
            "Please specify which synthesizer you want to use: A - synthesizer 1; B - synthesizer 2."
        )
    else:
        return args.synth
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/dev/ttyUSB0",
        help="Serial port for V5008.",
    )
    parser.add_argument(
        "--baud", dest="baud", type=int, default=9600, help="Baud rate."
    )
    parser.add_argument(
        "--synth",
        dest="synth",
        ...
        choices=["A", "B"],
        default=None,
        help="A - synthesizer 1; B - synthesizer 2.",
    )
    parser.add_argument(
        "--freq", dest="freq", type=float, help="The frequency in MHz."
    )
    parser.add_argument(
        "--amp",
        dest="amp",
    ...
    parser.add_argument(
        ...
        default=None,
        help="The reference source('internal' or 'external')",
    )
    parser.add_argument(
        "--label",
        dest="label",
        ...
        const="",
        default=None,
        help="Set the synthesizer label; omit value to get the current label.",
    )
    parser.add_argument(
        "--status",
        dest="status",
        default=False,
        action="store_true",
        help="Check the synthesizer status",
    )
    parser.add_argument(
        "--flash",
        dest="flash",
        default=False,
        action="store_true",
        help="Write the parameters into flash",
    )
    parser.add_argument(
        "--eigsep",
        dest="eigsep",
        default=False,
        action="store_true",
        help="Apply EIGSEP default configuration (A=500 MHz, B=250 MHz, external ref, 5 dBm, flash)",
    )
